...
const PlayerFinderPage: React.FC = () => {
  ...
  const { searchResults, nearbyPlayers, loading, error, pagination } = useSelector((state: RootState) => state.playerFinder);
  ...
  const [privacySettings, setPrivacySettings] = useState({
    can_be_found: user?.can_be_found ?? true,
    showEmail: true,
    showPhone: true,
    showLocation: true,
    allowContact: true
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          toast.info('Location access is needed for better search results');
        }
      );
    }
  }, []);
  ...
  // Fetch nearby players on mount
  useEffect(() => {
    if (currentLocation) {
      handleSearchPlayers();
    } else {
      dispatch(fetchNearbyPlayers(20));
    }
  ...
  const handleSearchPlayers = async () => {
    try {
      ...
      await dispatch(searchPlayers(searchParams)).unwrap();
    } catch (error) {
  ...
  const handleContactPlayer = (player: any) => {
    setSelectedPlayer({
      ...
      isVisible: true,
      lastActive: player.last_login || new Date().toISOString(),
      bio: player.bio || '',
      rating: 0
    });
  ...
  const updatePrivacySettings = async () => {
    try {
      await dispatch(updateProfile({ can_be_found: privacySettings.can_be_found })).unwrap();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-4 flex justify-between items-center">
          <p className="text-sm text-gray-600">
            Found {searchResults.length || nearbyPlayers.length} player{(searchResults.length || nearbyPlayers.length) !== 1 ? 's' : ''} 
            {currentLocation ? ' in your area' : ''}
          </p>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {(searchResults.length > 0 ? searchResults : nearbyPlayers).map((player: any) => (
              ...
              <div className="p-6">
                <div className="flex items-center space-x-4 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-semibold text-gray-900 truncate">{player.full_name || player.username}</h3>
                    {player.skill_level && (
                    ...
                    {player.rating && (
                      <div className="flex items-center mt-1">
                        <div className="flex items-center">
                          {[...Array(5)].map((_, i) => (
                            <svg key={i} className={`h-4 w-4 ${i < Math.floor(player.rating || 0) ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900">{player.city}, {player.state}</p>
                    {player.distance_km && (
                      <p className="text-xs text-gray-500">{player.distance_km} km away</p>
                    )}
                  </div>
                  ...
                  {player.membership_status && (
                    <div>
                      ...
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                        player.membership_status === 'premium' ? 'bg-purple-100 text-purple-800' :
                        player.membership_status === 'elite' ? 'bg-gold-100 text-gold-800' :
                        'bg-gray-100 text-gray-800'
                      }`}>
                        {player.membership_status.charAt(0).toUpperCase() + player.membership_status.slice(1)}
                      </span>
                    </div>
                  )}
                      ...
                      <div className="flex items-center">
                        <div className="flex-1 bg-gray-200 rounded-full h-2">
                          <div 
                            ...
                            style={{width: `${Math.min(player.matchScore, 100)}%`}}
                          ></div>
                        </div>
                        <span className="ml-2 text-xs text-gray-600">{player.matchScore}%</span>
                      </div>
                    </div>
                  )}

                  <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                    <span className="text-xs text-gray-500">Last active: {player.last_login ? new Date(player.last_login).toLocaleDateString() : 'Never'}</span>
                    <button
        ...
        {!loading && searchResults.length === 0 && nearbyPlayers.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">Try adjusting your search criteria or allow location access for better results.</p>
          </div>
        )}
          ...
          <div className="flex justify-center mt-8">
            <div className="flex space-x-2">
              ...
              <span className="px-3 py-2 text-sm text-gray-700">
                Page {searchPage} of {pagination.pages}
              </span>
              <button
                onClick={() => {
                  if (searchPage < pagination.pages) {
                    setSearchPage(searchPage + 1);
                    handleSearchPlayers();
                  }
                }}
                disabled={searchPage === pagination.pages}
                className="px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50"
              >
                Next
              </button>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <label className="flex items-center">
                    <input
                      type="checkbox"
                      checked={privacySettings.can_be_found}
                      onChange={(e) => setPrivacySettings({...privacySettings, can_be_found: e.target.checked})}
                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                    />
                    <span className="ml-2 text-sm font-medium text-gray-700">Can Be Found in Search</span>
                  </label>
                  <p className="text-xs text-gray-500 ml-6">When enabled, other players can find you in their searches. When disabled, you will not appear in any search results.</p>
